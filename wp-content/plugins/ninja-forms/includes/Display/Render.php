...
class NF_Display_Render
{
    ...
    public static function localize( $form_id )
    {
        ...
        $form->update_setting( 'currency_symbol', self::getCurrencySymbol($currency_symbol,$currency) );
    ...
    {
        $return = apply_filters('ninja_forms_localize_fields', $field);
        $return = apply_filters('ninja_forms_localize_field_' . $field_type, $field);
     ...
     * 
     * After any WP filter, we cannot assume that all properties are intact, so ensure that our structure is valid.
     *
     * @param array $field
     ...
     * @return string
     */
    protected static function getCurrencySymbol($currencySymbol, $currency): string
    {
        if(!is_string($currency)){
            return '';
        }
        
        $return = isset( $currencySymbol[ $currency ] ) ? $currencySymbol[ $currency ] : '';
